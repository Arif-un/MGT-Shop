...
class p_details_cus_in
{
    public function checkProductExistence($date,$cusname,$invoice,$pd_name)
    {
        $db = new database();

        $sql = "SELECT * FROM `p_transaction` WHERE `date`= ? AND `cus_name`= ? AND `invoice_no` = ? AND `pd_name`= ?";
    ...
    public function insert($data)
    {
        ...
        $sql = "INSERT INTO `p_transaction`(`date`,cus_name,pd_name,pd_code,quantity,unit,payment,price,amount,invoice_no,delivary_invoice_no) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public function total($invoice)
    {
        ...
        $sql = "SELECT sum(amount) as total FROM `p_transaction` WHERE invoice_no='$invoice'";
    ...
    public function discount($invoice)
    {
        ...
        $sql = "SELECT `discount` FROM `p_cus_total_tran` WHERE `id`='$invoice'";
    ...
    public function update_amnt($total,$dis,$vat,$n_total,$id)
    {
        ...
        $sql = "UPDATE `p_cus_total_tran` SET `total`=?,`discount`=?,`vat`=?,`net_total`=? WHERE `id`= ?";
    ...
    public function delete($id)
    {
        ...
        $sql = "DELETE FROM `p_transaction` WHERE `id` = '$id'";
    ...
    public function edit($quan,$price,$amount,$id)
    {
        ...
        $sql = "UPDATE `p_transaction` SET `quantity`=?,`price`=?,`amount`=? WHERE id= ?";
    ...
    public function update($payment,$del,$dis,$invoice)
    {
        ...
        $sql = "UPDATE `p_transaction` SET `payment`='$payment',`delivary_invoice_no`='$del' WHERE invoice_no= '$invoice'";
        ...
        $sql2 = "UPDATE `p_cus_total_tran` SET `discount`='$dis',`payment`='$payment',`delivary_invoice_no`='$del' WHERE id='$invoice'";
